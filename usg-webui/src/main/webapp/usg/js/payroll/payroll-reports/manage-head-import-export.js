 ...
 */
var exportData;
function displaypagemanageHeadImportandExport(divid) {
    createsearchEmployeeHeadForm(divid);
    createHeadimportForm();
   
}
function createHeadimportForm(){
    ...
    $("#employeeimportFormHeader").append("<class='panel-title' style='font-weight:bold;font-size:15px;' data-parent='#accordion2'><center>Head  Import </center><div class='pull-right' style='position: relative;bottom: 15px;cursor:pointer;' id='colHeadImport'><span class='glyphicon glyphicon-minus-sign'></span></div></div>");
    $("#employeeimportFormpanelDiv").append("<div id='collapseOne4' class='panel-collapse collapse in pal' />");
    $("#colHeadImport").click(function() {
        $("#collapseOne4").toggle();
        if ($("#colHeadImport span").hasClass("glyphicon-minus-sign")) {
            $("#colHeadImport").text("").append("<span class='glyphicon glyphicon-plus-sign'></span>");
        } else {
            $("#colHeadImport").text("").append("<span class='glyphicon glyphicon-minus-sign'></span>");
        }
    });
...
$("#employeeimportFormFormGroupDiv").append("<div class='col-lg-2'><a href='../../usg/DownloadFiles/salaryimport.xlsx' download><u><span style='color:green'>Sample Template</span></u></a>");
...
function  createsearchEmployeeHeadForm(divid) {
    //Start Title Bar
  $("#dashboardTitleMainDiv").text("").append("Payroll Reports");
    $("#dashboardOrderListId").text("").append('<a href="javascript:void(0)" data-toggle="tab" onclick="displayPayrollHorizontalBar()">Payroll </a> ><label><a href="javascript:payrollReportsMenuTabs()">Head Import and Export</label>');
    //End  Title Bar
    //End  Title Bar
    ...
    $("#employeeimportsearchtableHeader1").append("<class='panel-title' a data-toggle='collapse' style='font-weight:bold;font-size:15px;' data-parent='#accordion2' href='#collapseOne2'><center>Head Export & Import</center>");
    ...
    $("#panelRow1").append("<div  class='col-xs-12' id='buttonRow'/><center><input type='hidden' id='idValue'><button type='button'  id='employeeimportsearchSaveandUpdateButton' value='view' class='btn btn-success mr5'  onclick='validationsearchEmployeeData()'>View</button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp\n\
    <button type='button' id='employeeimportsearchResetButton' onclick='wipeAllemployeeimportsearchData()' class='btn btn-warning mr5' name='reset' value='reset'>Reset</button></center></div>");

}
//start validatio
function validationsearchEmployeeData() {
...
function displaySearchTable() {
    ...
    $("#employeeimportsearchListPanelRow").append("<div  id='tablesuccessBefore'/>");
     $('#employeeimportsearchListPanelTableMainDiv').append('<div class="btn-group mtm mbm"><a href="#" data-toggle="dropdown"   class="btn btn-success btn-sm dropdown-toggle">\n\
                   ...
                   $("#exportDiv").append("<li><a href='javascript:void(0);' onclick='exportManageHeadData();'><img src='../../assets/vendors/tableExport/icon/xls.png' width='24px' class='mrx'/>Export  to Excel</a></li>");
    ...
    $("#example1").append("<thead class=''><tr>"
            ...
            + " <th> S.No</th>"
            + "<th style='min-width:30%;width:auto;'><i class='fa'></i>Employee Code</th>"
...
function importFileValidation(){
   ...
   if ($('#importFile').get(0).files.length === 0) {
    ...
    xhr.open("POST", server_base_url+"payroll/managehead/Import", true);
...
function wipeAllemployeeimportsearchData(){
    displayEmployeeImportandExportPage("dashboardBodyMainDiv");
}

function exportManageHeadData(){
    ...
    if(length>0){
    $('#example1Body tr input[type="checkbox"][name="case"]:checked').each(function(i) {
      ...
      $.post(server_base_url + "/payroll/managehead/Export",{
        obj:obj,
        }).done(function (data) {
         if (data == fail) {
