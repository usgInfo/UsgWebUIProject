 ...
 */
function extraProvisionExpenseBudget() {
    scrolupfunction();
    $("#dashboardTitleMainDiv").text("").append('<label><a href="javascript:displayBudgetHorizontalBar()">Budget</a></label>');
    $("#dashboardOrderListId").text("").append('<label><a href="javascript:displayBudgetHorizontalBar()">Budget</a></label> >> <label><a href="javascript:budgetTransactionMenuTabs()">Budget Transaction</a></label> >> <label>Extra Provision Expense</label>');
    ...
    $("#tableHeader1").append("<div class='panel-title' a data-toggle='collapse' style='font-weight:bold;font-size:15px;' data-parent='#accordion2' href='#collapseOne2'><center>Extra Provision Expense</center><div class='pull-right' style='position: relative;bottom: 15px;cursor:pointer;' id='collapseFin'><span class='glyphicon glyphicon-minus-sign'></span></div></div>");
    ...
    $("#panelRow4").append("<div  class='col-xs-12' /><center><button type='button'  value='Save' class='btn btn-success mr5'  onclick='extraProvisionExpenseValidation()'>Create</button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<button type='button' onclick='wipeAllIncomeUniversitysBudgetCodeData()' class='btn btn-warning mr5 ' name='reset' value='reset'>Reset</button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<button type='button' onclick='searchPanelExtraProvisionExpense()' class='btn btn-warning mr5' name='reset' value='reset'>Search</button></div>");
    getBudgetfinancialyearForIncomeUniversity("financialYear");
    $("#budgetType").attr("onchange", " fetchDepartInExtraProvisionExpense('Department','department')");
    ...
    $("#budgethead").attr("onchange", " fetchDepartInExtraProvisionExpensewithBudethead('Department','department')");
    getBudgetTypeExtraProvision('budgetType');

}
function fetchDepartInExtraProvisionExpense(message, DivId) {
    var result = 1;
...
function fetchDepartInExtraProvisionExpensewithBudethead(message, DivId) {
    var result = 1;
    if ($('#financialYear').val() == null || $('#financialYear').val() == "") {
        $("#financialYear").focus();
        displaySmallErrorMessages("financialYearErr", "Please Select Financial Year.");
...
function fetchDepartInExtraProvisionExpenseSearch(message, DivId) {
    var result = 1;
...
function searchPanelExtraProvisionExpense() {
    $("#searchPanel").remove("");
    ...
    $("#savesearchButton").append("<button class='btn btn-success mr5 btn-flat' id='SearchIncomeBudgetButton' onclick='validateExtraProvisionExpenseSearch()'>Search</button>&nbsp&nbsp&nbsp&nbsp&nbsp");
    ...
    $("#budgetTypeSearch").attr("onchange", " fetchDepartInExtraProvisionExpenseSearch('Department','departmentSearch')");
...
function extraProvisionExpenseValidation() {
...
function viewExtraProvisionExpenseList(divId)
{
    ...
    $.get(server_base_url + "/budget/budgetTransaction/ExtraProvisionExpenseSearch", {
            ...
            if (bdata != null) {
                if (bdata.length > 0) {
                    ...
                    for (var i = bdata.length - 1; i >= 0; i--) {
                        ...
                        if (result != 0) {
                            ...
                            $("#displayBankTableBody").append("<tr id='" + bdata[i]._id.$oid + "' style='cursor:pointer;' >"
                                    ...
                                    + "<td style='cursor:pointer;'>" + bdata[i].ledgerName + "<input type='hidden' value='" + bdata[i].budgetDate + "'></td>"
                    ...
                    if (sno > 0) {
                        ...
                        $("#saveSubmitResetPrintRow").append('<center><button type="button"  value="Save" class="btn btn-success mr5 btn-flat"  onclick=saveExtraProvisionexpense("' + save + '")>Save</button>&nbsp;&nbsp;&nbsp;<button name="reset" value="reset" type="button" class="btn btn-warning mr5" onclick=ResetExtraProvisionIncome()>Reset</button></center>');
                    }
...
function saveExtraProvisionexpense(saveorsubmit) {
    ...
    if (status == "false")
    {
        ...
        if (result != 0)
        {
            ...
            $.post(server_base_url + "/Budget/BudgetTransaction/ExtraProvisionExpenseSave", {
...
function validateExtraProvisionExpenseSearch() {
    ...
    if (result != 0) {
        ...
        $("#viewUser").append("<div class='table-responsive' id='viewUserSectionTableDiv' />");
        $("#viewUserSectionTableDiv").append("<table class='table table-bordered table-striped table-warning mb30' id='displayBankTable' />");
        $.post(server_base_url + "/budget/budgetTransaction/ExtraProvisionExpenseSearch", {
            searchObj: JSON.stringify(searchObj),
                ...
                if (universityBudgetStatus == "Save") {
                    if (bdata.length > 0) {
                        ...
                        for (var i = bdata.length - 1; i >= 0; i--) {
                            ...
                            $("#" + sno).append("<td style='cursor:pointer;' onclick=UpdateExtraProvisonExpenseData('" + encodeURI(objJson) + "')>" + ' <i class="fa fa-edit"></i>&nbsp;&nbsp;<a  class="anchor_edit" style="margin-width:1%,width:80px" >Edit</a>' + "</td>");
                            $("#" + sno).append("<td onclick=deletePopUp('deleteExtraProvisionExpense','nothingToDo','" + bdata[i]._id.$oid + "')>" + ' <i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a  class="anchor_delete" style="margin-width:1%,width:80px" >Delete</a>' + "</td></tr>");
                            $("#displayBankTable thead tr th:first input:checkbox").change(function() {
                                $("#displayBankTable tbody tr td:first-child input:checkbox").prop('checked', $(this).prop("checked"));
                            });
                            $("#displayBankTable tbody tr td:first-child input:checkbox").change(function() {
                        ...
                        if (sno > 0) {
                            ...
                            $("#saveSubmitResetPrintRow").append("<div class='col-sm-1'><button class='btn btn-success mr5 btn-flat'  onclick='SubmitExtraProvisionExpense()'>Submit</button></div>");
...
function UpdateExtraProvisonExpenseData(obj) {
    $("#messageDiv").text("");
    obj = decodeURI(obj);
    obj = JSON.parse(obj);
    $(".smallErrorMsg").text("");
    ...
    $("#saveSubmitResetPrintRow").append("<div class='col-sm-3'></div>");
    $("#saveSubmitResetPrintRow").append("<div class='col-sm-1'></div>");
    $("#saveSubmitResetPrintRow").append("<div class='col-sm-1'><button class='btn btn-success mr5 btn-flat'  onclick='UpdateExtraProvisionExpenseRow()'>Update</button></div>");
...
function UpdateExtraProvisionExpenseRow() {
    var id;
    ...
    $.post(server_base_url + "/Budget/BudgetTransaction/ExtraProvisionExpenseUpdate", {
        Id: id,
...
function SubmitExtraProvisionExpense() {
    var result = 1;
    ...
    if (result != 0)
    {
        ...
        $.post(server_base_url + "/Budget/BudgetTransaction/ExtraProvisionExpenseSubmit", {
...
function deleteExtraProvisionExpense(Id) {
    $.post(server_base_url + "/Budget/BudgetTransaction/ExtraProvisionExpenseDelete", {
        Id: Id,
        userid: getUserSessionElement("userId")
    }).done(function(data) {
        if (data == fail) {
