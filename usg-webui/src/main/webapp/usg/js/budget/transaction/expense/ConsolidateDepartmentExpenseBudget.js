 ...
 */
function ConsolidateDepartmentExpenseBudget(divId) {
    scrolupfunction();
    $("#dashboardTitleMainDiv").text("").append('<label><a href="javascript:displayBudgetHorizontalBar()">Budget</a></label>');
    $("#dashboardOrderListId").text("").append('<label><a href="javascript:displayBudgetHorizontalBar()">Budget</a></label> >> <label><a href="javascript:budgetTransactionMenuTabs()">Budget Transaction</a></label> >> <label>Consolidate Expense Budget @ Department Head</label>');
    ...
    if (checkUserPrivelege(pvCreateBudgetConsolidatedIncome)) {
        ...
        $("#tableHeader1").append("<class='panel-title' a data-toggle='collapse' style='font-weight:bold;font-size:15px;' data-parent='#accordion2' href='#collapseOne2'><center>Consolidate Expense Budget @ Department Head</center><div class='pull-right' style='position: relative;bottom: 15px;cursor:pointer;' id='collapseFin'><span class='glyphicon glyphicon-minus-sign'></span></div></div>");
        ...
        $("#panelRow4").append("<div  class='col-xs-12' /><center><button type='button'  value='Save' id='createIncomeBudgetButton' class='btn btn-success mr5 '  onclick='consolidateDeptexpenseBudgetValidation()'>Create</button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<button type='button' onclick='wipeAllFinancialConsolidateIncomeBudgetCodeData()' class='btn btn-warning mr5 ' name='reset' value='reset'>Reset</button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<button type='button' onclick='searchforConsolidateDeptExpenseBudget()' class='btn btn-warning mr5' name='reset' value='reset'>Search</button></center></div>");
...
function consolidateDeptexpenseBudgetValidation() {
    ...
    if (result != 0) {
        viewConsolidateDepartmentExpense();
    }
    //  }
}
function searchforConsolidateDeptExpenseBudget() {
    if (checkUserPrivelege(pvViewBudgetConsolidatedIncome)) {
        ...
        $("#savesearchButton").append("<button class='btn btn-success mr5 btn-flat' id='SearchIncomeBudgetButton' onclick='validateConsolidateDeptExpenseSearch()'>Search</button>&nbsp&nbsp&nbsp&nbsp&nbsp");
...
function validateConsolidateDeptExpenseSearch(previousSearchJson) {
    if (checkUserPrivelege(pvViewBudgetConsolidatedIncome)) {
...
//        }
        ...
        if (result != 0) {
            ...
            $.post(server_base_url + "budget/transaction/ConsolidateDeptExpense/Search", {
...
//                                    + "</tr></thead>");
                            ...
                            for (var i = bdata.length - 1; i >= 0; i--) {
                                ...
                                if (checkUserPrivelege(pvUpdateBudgetConsolidatedIncome)) {
                                    $("#" + sno).append("<td style='cursor:pointer;' onclick=updateConsolidateDeptExpense('" + encodeURI(objJson) + "')>" + ' <i class="fa fa-edit"></i>&nbsp;&nbsp;<a  class="anchor_edit" style="margin-width:1%,width:80px" >Edit</a>' + "</td>");
                                }
                                if (checkUserPrivelege(pvDeleteBudgetConsolidateIncome)) {
                                    $("#" + sno).append("<td onclick=deletePopUp('deleteConsolidateDeptExpense','nothingToDo','" + bdata[i]._id.$oid + "')>" + ' <i class="fa fa-trash-o"></i>&nbsp;&nbsp;<a  class="anchor_delete" style="margin-width:1%,width:80px" >Delete</a>' + "</td></tr>");
                                }
                            ...
                            if (sno > 0) {
                                ...
                                if (checkUserPrivelege(pvCreateBudgetConsolidatedIncome) || checkUserPrivelege(pvUpdateBudgetConsolidatedIncome)) {
                                    ...
                                    $("#saveSubmitResetPrintRow").append("<div class='col-sm-1'><button class='btn btn-success mr5 btn-flat'  onclick='SubmitConDeptExpenseSearch()'>Submit</button></div>");
                                    $("#saveSubmitResetPrintRow").append("<div class='col-sm-1'><button class='btn btn-warning mr5 btn-flat' onclick=ConsolidateDepartmentExpenseBudget('dashBoardBodyMainDiv') >Reset</button></div>");
                                    $("#saveSubmitResetPrintRow").append("<div class='col-sm-1'></div>");
                                }
                            }
...
function viewConsolidateDepartmentExpense(divId)
{
    ...
    if (checkUserPrivelege(pvViewBudgetConsolidatedIncome)) {
        ...
        setTimeout(function() {
            $.get(server_base_url + "budget/transaction/ConsolidateDeptExpense/Search", {
                searchObj: JSON.stringify(searchObj),
                    ...
                    if (bdata != null) {
                        if (bdata.length > 0) {
                            ...
                            if (sno > 0) {
                                ...
                                if (checkUserPrivelege(pvCreateBudgetConsolidatedIncome)) {
                                    $("#listpanelRow").append("<div class='row' id='saveSubmitResetPrintRow'/>");
                                    $("#saveSubmitResetPrintRow").append('<center><button type="button"  value="Save" class="btn btn-success mr5 btn-flat"  onclick=saveConsolidateDeptExpense("' + save + '")>Save</button>&nbsp;&nbsp;&nbsp;<button name="reset" value="reset" type="button" class="btn btn-warning mr5" onclick=resetTextFieldsofTable("displayBankTable","5")>Reset</button></center>');
                                }
                            }
                            $('#displayBankTable').dataTable();
                        } else {
...
function saveConsolidateDeptExpense(saveorsubmit, searchSubmit) {
    if (checkUserPrivelege(pvCreateBudgetConsolidatedIncome)) {
        var status = "false";
        ...
        if (status == "false")
        {
            ...
            if (result != 0)
            {
                $.post(server_base_url + "budget/transaction/ConsolidateDeptExpense/Save", {
                    ...
                    if (data == fail) {
                        ...
                        setTimeout(function() {
                            ...
                            wipeAllFinancialConsolidateIncomeBudgetCodeData();
                        }, 3000);
...
function SubmitConDeptExpenseSearch() {
    if (checkUserPrivelege(pvUpdateBudgetConsolidatedIncome)) {
        var result = 1;
        var saveThisConsolidateDetails = [];
        var rows = $("#displayBankTable").dataTable().fnGetNodes();
        ...
        if (result != 0)
        {
            ...
            $.post(server_base_url + "budget/transaction/ConsolidateDeptExpense/Submit", {
                objJson: JSON.stringify(saveThisConsolidateDetails),
                userid: userid
            }).done(function(data) {
                if (data == fail) {
...
function updateConsolidateDeptExpense(obj) {
    $("#messageDiv").text("");
    obj = decodeURI(obj);
    obj = JSON.parse(obj);
    $("#tableHeaderTable").text("").append("<div id='maritalListPanelSearch' class='panel panel-blue' />");
    ...
    $("#buttonIdofTable").append("<center><button class='btn btn-success mr5 btn-flat'  onclick='UpdateCinsolidateDeptExpenseRow()'>Update</button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<button class='btn btn-warning mr5 btn-flat' onclick='goBackToConsolidateSearchFunction()'>Back</button></center>");
}
function UpdateCinsolidateDeptExpenseRow() {
    if (checkUserPrivelege(pvUpdateBudgetConsolidatedIncome)) {
        var id;
        var askedForMoney;
        ...
        $.post(server_base_url + "budget/transaction/ConsolidateDeptExpense/Update", {
            Id: id,
...
function deleteConsolidateDeptExpense(Id) {
    if (checkUserPrivelege(pvDeleteBudgetConsolidateIncome)) {
        $.post(server_base_url + "budget/transaction/ConsolidateDeptExpense/Delete", {
            Id: Id,
            userid: getUserSessionElement("userId")
        }).done(function(data) {
            if (data == fail) {
