    ...
    if (checkUserPrivelege(pvCreateContraVoucher)) {
        ...
        $("#colfinContraVoucher").click(function () {
            ...
            if ($("#colfinContraVoucher span").hasClass("glyphicon-minus-sign")) {
                ...
                $("#colfinContraVoucher").text("").append("<span class='glyphicon glyphicon-minus-sign'></span>");
            }
        });
        ...
        $("#FieldGroup").append('<div class="col-lg-12"><div class="form-group col-lg-6" id="ddoerr"><label for="ddo">DDO <span class="require">*</span> </label><select class="form-control" name="ddo" id="DDO" onchange=getLocationBasedOnDDOInContraVoucher()></select>'
        ...
        $("#firstHeaderOfReceiptVoucherListAdd").append("<div  style='font-weight:bold;font-size:15px;' data-parent='#accordion2' ><center>List of Ledger Amount Detail(s)</center>");
...
//        $("#panelRow4").append("<div  class='col-xs-3' /><div class='col-xs-3'><button type='button' style='margin-left:10px' value='Add' id='addButtonatSave' class='btn btn-success mr5 pull-right' onclick=addContralVoucherRows('displayReceiptVoucherAddTableBody')>Add</button></div>");
        ...
        $("#panelRowNarrationvalues").append('<div class="col-lg-12 narrationval" ><div class="form-group col-lg-6"><label for="fundtype">Narration Type <span class="require">*</span> </label><select class="form-control" name="narrationType" id="narrationType" onchange=getShortdescnarrationContra()></select></div>');
...
//    });
        ...
        $("#firstHeaderOfReceiptVoucherListAdd1").append("<div  style='font-weight:bold;font-size:15px;' data-parent='#accordion2' ><center>Payment Mode  </center></div>");
        ...
        $("#panelRowPaymentMode").append("<div class='col-sm-2'/><label  class='col-sm-1' >By Cheque</label><input  type='radio' id='paymentModeRadioButton1' name='paymentmoderadiobutton'   onclick=addChequeDetailRowContra('panelRowPaymentModeDivision1','','','','','','','" + fromFinacialYear + "','" + toFinacialYear + "') class='col-sm-1'><div class='col-sm-2'/><label class='col-sm-1'>Bank Transfer</label><input type='radio' id='paymentModeRadioButton2' onclick=addBankDetailRowContra('panelRowPaymentModeDivision1') name='paymentmoderadiobutton'  class='col-sm-1'></div>");
        ...
        $("#panelRow5").append("<div  class='col-xs-12'><center><button type='button' style='margin-left:10px' value='Save' class='btn btn-success mr5' onclick='contraVoucherValidation()'>Save</button>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<button type='button' onclick='contraVoucherMaster()' class='btn btn-warning mr5' name='reset' value='reset'>Reset</button></center></div>");
        //$("#panelRow5").append("<div  class='col-xs-3' /><div class='col-xs-2'><button type='button' style='margin-left:10px' value='Save' class='btn btn-success mr5 pull-right' onclick='contraVoucherValidation()'>Save</button></div><div class='col-xs-2'><button type='button' onclick='resetContraVoucherList()' class='btn btn-warning mr5 pull-left' name='reset' value='reset'>Reset</button></div>");    
    ...
    if (checkUserPrivelege(pvViewContraVoucher)) {
        SearchContraVoucher();
        //For List
        ...
        $("#colfincontraVoucherSearch").click(function () {
            ...
            if ($("#colfincontraVoucherSearch span").hasClass("glyphicon-minus-sign")) {
                ...
                $("#colfincontraVoucherSearch").text("").append("<span class='glyphicon glyphicon-minus-sign'></span>");
            }
        });
...
function getNarrationListinContra(name)
{
...
function getShortdescnarrationContra()
{
...
function addBankDetailRowContra(DivId, bankAccountNo, bankName, bankAmount, bankRemarks) {
    rat = 0;
...
function addChequeDetailRowContra(DivId, chequeNo, chequeDate, chequeBankName, chequeAmount, inFavorOf, checkRemarks, fromFinacialYear, toFinacialYear) {
    rat = 0;
    if (chequeNo != null) {
            ...
            for (var i = 0; i < ledgerList.length; i++) {
                ...
                $("#displayReceiptVoucherAddTableBody").on('keydown', "#" + narrationId, function (e) {
                    ...
                    if (keyCode == 9) {
                        ...
                        addRowContraUpdateinVouchers('displayReceiptVoucherAddTableBody');
                    }
                });
                ...
                disablefunctionForDrCrForContraVoucher(drCrId, drAmountId, crAmountId);
            }
...
//                    + "<td style='cursor:pointer;'><input type=''text' placeholder='Short Narration'  class='form-control'></td>"
//                    + "<td style='cursor:pointer;'>" + ' <button type="button" class="btn mr5 btn-primary" onclick=deleteContraVoucherListRow(this.parentNode.parentNode.rowIndex,"' + drAmountId + '","' + crAmountId + '") style="align:center;color:white">DELETE</button>' + "</td>"
//                    + "</tr></thead>");
//
//            $("#" + drCrId).attr("onchange", "getGroupListForOptionsContra('','" + groupId + "', 'Group','" + drCrId + "'); onChange=disablefunctionForDrCrForContraVoucher('" + drCrId + "','" + drAmountId + "','" + crAmountId + "')");
//
//            getGroupListForOptionsContra("", groupId, "Group");
//            $('.drAmount').keyup(function() {
//                var sum = 0;
...
function addRowContraUpdateinVouchers(DivId)
{
    if (($('#totalDrAmount').val() == $('#totalCrAmount').val()) && ($('#totalDrAmount').val() != undefined && $('#totalCrAmount').val() != 0))
    {
        displayLargeErrorMessagesInCenterInRed("addbuttonmsg", "Both Debit and Credit Amount are Same");
...
function getLedgerListForContra(id, ledgerId, ledgerval, groupid)
{
    var group;
...
function disablefunctionForDrCrForContraVoucher(drCrId, drAmountId, crAmountId) {
    if ($("#" + drCrId).val() == "Cr") {
...
function deleteContraVoucherListRow(i, dramount, cramount) {
...
function resetContraVoucherList()
{
...
function contraVoucherValidation() {
    ...
    if (result == 1) {
        ...
        if ($("#saveorupdate").val() == "save") {
            saveContraVoucherDetails();
            globalTotalBankAmount = 0;
            globalTotalChequeAmount = 0;
        } else if ($("#saveorupdate").val() == "update") {
            updateContraVoucherDetails();
            globalTotalBankAmount = 0;
            globalTotalChequeAmount = 0;
        }
    } else if (result == 5) {
...
function getLocationBasedOnDDOInContraVoucher(name, ddoId) {
    var ddo = $("#DDO").val();
    if (ddo == null)
    {
        ddo = ddoId
...
//            bankRemarks: $("#bankRemarks").val(),
        ...
        $.post(server_base_url + "/financial/transaction/ContraVoucher/Save", {
...
function viewContraVoucherListMaster(DivId) {
    if (checkUserPrivelege(pvViewContraVoucher)) {
        ...
        $("#displayBankTable").append("<thead class=''><tr id='displayBankTableTrHeadCV'>"
        ...
        if (checkUserPrivelege(pvUpdateContraVoucher)) {
            $("#displayBankTableTrHeadCV").append("<th style='min-width:1%;width:80px;''><i class='fa'></i>Edit</th>");
        }
...
//                    for (var i = 0; i < bdata.length; i++) {
                        for (var i = bdata.length - 1; i >= 0; i--) {
                            ...
                            if (checkUserPrivelege(pvUpdateContraVoucher)) {
                                $("#" + bdata[i]._id.$oid).append("<td style='cursor:pointer;' onclick=updateContraVoucher('" + encodeURI(objjson) + "')>" + ' <i class="fa fa-edit"></i>&nbsp;&nbsp;<a  class="anchor_edit">Edit</a>' + "</td>");
                            }
                            //   + "<td style='cursor:pointer;' onclick=updateContraVoucher('" + encodeURI(objjson) + "')>" + ' <button type="button"  class="btn btn-primary mr5"style="align:center;color:white"  >EDIT</button>' + "</td>"
                            //  + "<td onclick=deleteContraVoucher('" + bdata[i]._id.$oid + "')>" + ' <button type="button" class="btn btn-danger mr5"  style="align:center;color:white" >DELETE</button>' + "</td></tr>");
                        }
...
function deleteContraVoucher(objId) {
    if (checkUserPrivelege(pvDeleteContraVoucher)) {
...
function  updateContraVoucher(obj) {
    ...
    globalTotalChequeAmount = 0;
    viewContraVoucherAddList('receiptVoucherlistAddpanelRow');
    addContralVoucherRows("displayReceiptVoucherAddTableBody", obj.ledgerList);
    var isGrant = "";
    if (obj.isGrant == "Yes") {
    ...
    //  $("#panelRow4").text("").append("<div class='col-sm-6'><button type='button'   class='btn btn-success pull-right mr5'  onclick=addContralVoucherRows('displayReceiptVoucherAddTableBody')>Add</button></div>");
    ...
    getNarrationListinContra(obj.narrationType);
...
function SearchContraVoucher() {
    if (checkUserPrivelege(pvViewContraVoucher)) {
        ...
        $("#searchRow3").append("<div  class='col-xs-3' /><div class='col-xs-3'><button type='button' style='margin-left:10px' value='Save' class='btn btn-success mr5 pull-right' onclick='searchContraVoucherFunction()'>Search</button></div><div class='col-xs-2'><button type='button' onclick=resetAllValuesInSpecifiedDiv('searchlistpanelMainBody') class='btn btn-warning mr5 pull-left' name='reset' value='reset'>Reset</button></div>");
        //    $("#searchRow2").append("<center><button  onclick='searchContraVoucherFunction()' class='btn btn - success' style='height:40px; width:80px'>Search</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button onclick='resetSearchContraVoucherFunctionDeatils()' class='btn btn - warning'  style='height:40px; width:80px'>Reset</button></center>");
    }
}

function searchContraVoucherFunction() {
    if (checkUserPrivelege(pvViewContraVoucher)) {
...
//    }
            ...
            $.post(server_base_url + "/financial/transaction/ContraVoucher/Search", {
                fromDate: fromDate,
                ToDate: ToDate,
                voucherNo: voucherNo,
...
//                    for (var i = 0; i < bdata.length; i++) {
                            ...
                            for (var i = bdata.length - 1; i >= 0; i--) {
                                ...
                                if (checkUserPrivelege(pvUpdateContraVoucher)) {
                                    $("#" + sno).append("<td style='cursor:pointer;' onclick=updateContraVoucher('" + encodeURI(objjson) + "')>" + ' <i class="fa fa-edit"></i>&nbsp;&nbsp;<a  class="anchor_edit">Edit</a>' + "</td>");

                                }
                                // + "<td onclick=deleteContraVoucher('" + obj._id + "')>" + ' <button type="button" class="btn btn-danger mr5"  style="align:center;color:white" >DELETE</button>' + "</td></tr>");
                            }
                            $('#displayBankTable').dataTable();
                        }
                    } else {
...
function getHardCodedOptionsContraVoucher(name, DivId) {
